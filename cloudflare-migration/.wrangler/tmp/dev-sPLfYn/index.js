...
// .wrangler/tmp/bundle-NuYmSh/checked-fetch.js
...
// src/middleware/auth-helpers.ts
var auth_helpers_exports = {};
__export(auth_helpers_exports, {
  decodeClerkToken: () => decodeClerkToken
});
...
__export(email_templates_exports, {
  abandonedCartTemplate: () => abandonedCartTemplate,
  dailyReportTemplate: () => dailyReportTemplate,
  debtReminderTemplate: () => debtReminderTemplate,
  stockAlertTemplate: () => stockAlertTemplate,
...
function debtReminderTemplate(data) {
  let reminderMessage = "";
  let boxColor = "#fef2f2";
  let borderColor = "#dc2626";
  if (data.overdue_days && data.overdue_days > 0) {
    reminderMessage = `Te escribimos para recordarte que tienes un pago vencido desde hace ${data.overdue_days} ${data.overdue_days === 1 ? "d\xEDa" : "d\xEDas"}.`;
    boxColor = "#fef2f2";
    borderColor = "#dc2626";
  } else if (data.days_until_due !== void 0) {
    if (data.days_until_due === 0) {
      ...
      reminderMessage = "Te escribimos para recordarte que tu pago vence <strong>ma\xF1ana</strong>.";
      boxColor = "#fef3c7";
      borderColor = "#f59e0b";
    } else {
      reminderMessage = `Te escribimos para recordarte que tu pago vence en <strong>${data.days_until_due} d\xEDas</strong>.`;
      boxColor = "#fef9c3";
      borderColor = "#eab308";
    }
  } else {
  ...
  const content = `
    <div class="content">
      <h2 style="color: #1f2937; margin-top: 0;">Hola ${data.customer_name} \u{1F44B}</h2>
      ...
      <p style="color: #4b5563; font-size: 16px; line-height: 1.6;">
        Valoramos mucho tu confianza y queremos ayudarte a mantener tu cuenta al d\xEDa.
      ...
      <p style="color: #4b5563; font-size: 16px; line-height: 1.6; margin-top: 20px;">
        <strong>M\xE9todos de pago disponibles:</strong><br/>
        ${data.payment_methods}
      </p>
      ` : ""}
...
var init_email_templates = __esm({
  "src/utils/email-templates.ts"() {
    ...
    __name(debtReminderTemplate, "debtReminderTemplate");
  }
});

// .wrangler/tmp/bundle-NuYmSh/middleware-loader.entry.ts
init_checked_fetch();
init_modules_watch_stub();

// .wrangler/tmp/bundle-NuYmSh/middleware-insertion-facade.js
init_checked_fetch();
init_modules_watch_stub();
...
var Hono = class {
  ...
  route(path, app25) {
    ...
    app25.routes.map((r) => {
      let handler;
      if (app25.errorHandler === errorHandler) {
        handler = r.handler;
      } else {
        handler = /* @__PURE__ */ __name(async (c, next) => (await compose([], app25.errorHandler)(c, () => r.handler(c, next))).res, "handler");
  ...
  try {
    ...
    if (requestedTenantId) {
      const teamMember = await c.env.DB.prepare(`
        ...
        `).bind(clerkUserId, requestedTenantId).first();
      if (teamMember) {
        isTeamMember = true;
        ...
        if (!tenant) {
          tenant = {
            ...
            clerkUserId: "",
            // Will be filled from user_profile
            email: "",
            databaseName: "shared",
            databaseId: "shared",
        ...
        const ownerProfile = await c.env.DB.prepare("SELECT id FROM user_profiles WHERE clerk_user_id = ? AND id = ?").bind(clerkUserId, requestedTenantId).first();
        if (ownerProfile) {
          tenant = await tenantManager.getTenantByClerkId(clerkUserId);
          userProfileId = ownerProfile.id;
        } else {
          const superAdminProfile = await c.env.DB.prepare("SELECT id FROM user_profiles WHERE clerk_user_id = ? AND is_superadmin = 1").bind(clerkUserId).first();
      ...
      console.log("\u{1F50D} [authMiddleware] No X-Tenant-ID provided, checking if user is team member...");
      ...
      if (teamMembership) {
        console.log("\u{1F465} [authMiddleware] Usuario es team member de:", teamMembership.store_name);
        isTeamMember = true;
        ...
        console.log("\u{1F451} [authMiddleware] Usuario no es team member, buscando su propio tenant...");
        tenant = await tenantManager.getTenantByClerkId(clerkUserId);
      }
    }
    let clerkUser = null;
    ...
    if (userProfileId) {
      userProfile = await c.env.DB.prepare("SELECT id, is_superadmin, subscription_status FROM user_profiles WHERE id = ?").bind(userProfileId).first();
    } else {
      userProfile = await c.env.DB.prepare("SELECT id, is_superadmin, subscription_status FROM user_profiles WHERE clerk_user_id = ?").bind(clerkUserId).first();
    }
    if (!userProfile) {
      try {
        if (!clerkUser) {
        ...
        const existingProfileByEmail = await c.env.DB.prepare("SELECT id, clerk_user_id, email, is_superadmin, subscription_status FROM user_profiles WHERE email = ?").bind(userEmail).first();
    ...
    if (isTeamMember && userProfileId) {
      const ownerProfile = await c.env.DB.prepare("SELECT subscription_status FROM user_profiles WHERE id = ?").bind(userProfileId).first();
      if (ownerProfile) {
...
async function verifyClerkToken(token, env) {
  try {
    const { decodeClerkToken: decode } = await Promise.resolve().then(() => (init_auth_helpers(), auth_helpers_exports));
...
app2.post("/", async (c) => {
  ...
  try {
    ...
    if (!body.phone || body.phone.trim() === "") {
      return c.json({
        success: false,
        error: "El tel\xE9fono es requerido para enviar recordatorios de pago"
      }, 400);
    }
    ...
    if (!emailRegex.test(body.email)) {
      return c.json({
        success: false,
        error: "El email no tiene un formato v\xE1lido"
      }, 400);
    }
    ...
    if (phoneDigits.length < 7) {
      return c.json({
        success: false,
        error: "El tel\xE9fono debe tener al menos 7 d\xEDgitos"
      }, 400);
    }
    const tenantDB = new TenantDB(c.env.DB, tenant.id);
    const customerData = {
...
app3.get("/", async (c) => {
  ...
  try {
    const db = c.env.DB;
    const sales = await db.prepare("SELECT * FROM sales WHERE tenant_id = ? ORDER BY created_at DESC").bind(tenant.id).all();
    ...
    const salesWithItems = (sales.results || []).map((sale) => ({
      ...sale,
...
app3.get("/:id", async (c) => {
  ...
  try {
    const db = c.env.DB;
    const sale = await db.prepare("SELECT * FROM sales WHERE id = ? AND tenant_id = ?").bind(saleId, tenant.id).first();
    if (!sale) {
      return c.json({
        success: false,
...
app3.delete("/:id", async (c) => {
  ...
  try {
    ...
    const saleItems = await tenantDB.query("sale_items", "sale_id = ?", [saleId]);
    for (const item of saleItems) {
      ...
      if (product) {
        ...
        console.log(`\u{1F4E6} Stock restaurado para producto ${product.name}: ${product.stock} \u2192 ${restoredStock}`);
      }
    }
    if (sale.customer_id && sale.points_earned && sale.points_earned > 0) {
      const customer = await tenantDB.getById("customers", sale.customer_id);
      if (customer) {
        ...
        console.log(`\u{1F3AF} Puntos descontados del cliente: ${customer.loyalty_points} \u2192 ${updatedPoints}`);
      }
    }
    if (sale.customer_id && sale.payment_method === "credito" && sale.amount_pending && sale.amount_pending > 0) {
      ...
      if (customer && customer.current_debt) {
        ...
        console.log(`\u{1F4B0} Deuda actualizada del cliente: ${customer.current_debt} \u2192 ${updatedDebt}`);
      }
    }
    await tenantDB.db.prepare("DELETE FROM sale_items WHERE tenant_id = ? AND sale_id = ?").bind(tenant.id, saleId).run();
...
app6.get("/", async (c) => {
  ...
  try {
    ...
    const allSuppliers = await tenantDB.getAll("suppliers");
    const suppliersMap = /* @__PURE__ */ new Map();
...
app6.get("/:id", async (c) => {
  ...
  try {
    ...
    const supplier = await tenantDB.getById("suppliers", order.supplier_id);
...
app8.get("/", async (c) => {
  try {
    ...
    console.log("\u{1F50D} [GET /api/user-profiles] clerk_user_id:", clerkUserId);
    const teamMember = await c.env.DB.prepare(
    ...
    if (teamMember) {
      console.log("\u{1F465} [GET /api/user-profiles] Usuario es team member, retornando 404");
      return c.json({
        success: false,
    ...
    console.log("\u{1F4E6} [GET /api/user-profiles] Perfil encontrado:", !!result);
    if (!result) {
      return c.json({
        success: false,
...
app18.post("/debt-reminders", async (c) => {
  const db = c.env.DB;
  try {
    console.log("\u{1F514} Iniciando env\xEDo de recordatorios de deudas...");
    ...
    for (const store of stores.results) {
      try {
        const debtors = await db.prepare(
        ...
        for (const debtor of debtors.results) {
          try {
            const oldestSale = await db.prepare(
              `SELECT due_date
                 FROM sales
                 WHERE tenant_id = ?
                   AND customer_id = ?
            ...
            if (oldestSale?.due_date) {
              ...
              daysUntilDue = Math.floor(diffTime / (1e3 * 60 * 60 * 24));
              isOverdue = daysUntilDue < 0;
            }
            if (daysUntilDue <= 3) {
              const { debtReminderTemplate: debtReminderTemplate2 } = await Promise.resolve().then(() => (init_email_templates(), email_templates_exports));
              const emailData = {
                customer_name: debtor.name,
                debt_amount: debtor.current_debt,
                store_name: store.store_name || "Tu tienda",
                store_phone: store.phone,
                overdue_days: isOverdue ? Math.abs(daysUntilDue) : void 0,
                days_until_due: !isOverdue ? daysUntilDue : void 0,
                payment_methods: "Efectivo, Transferencia, Tarjeta"
              };
              const html = debtReminderTemplate2(emailData);
              const result = await sendEmail(
                {
                  to: debtor.email,
                  toName: debtor.name,
                  ...
                  subject: isOverdue ? `Recordatorio: Pago vencido - $${debtor.current_debt.toLocaleString("es-CO")}` : daysUntilDue === 0 ? `Recordatorio: Tu pago vence HOY - $${debtor.current_debt.toLocaleString("es-CO")}` : `Recordatorio: Tu pago vence en ${daysUntilDue} ${daysUntilDue === 1 ? "d\xEDa" : "d\xEDas"} - $${debtor.current_debt.toLocaleString("es-CO")}`,
                  html,
                  replyTo: store.email
                },
                c.env.RESEND_API_KEY
    ...
    console.log(
      `\u{1F389} Recordatorios de deudas completados: ${totalEmailsSent} enviados, ${totalEmailsFailed} fallidos en ${totalStoresProcessed} tiendas`
    );
    return c.json({
      success: true,
...
app21.get("/validate", async (c) => {
  try {
    ...
    const member = await c.env.DB.prepare("SELECT * FROM team_members WHERE invitation_token = ?").bind(token).first();
    if (!member) {
      return c.json({
        success: false,
        error: "Invitaci\xF3n no encontrada"
    ...
    if (member.invitation_status === "accepted") {
      return c.json({
        success: false,
        error: "Esta invitaci\xF3n ya fue aceptada"
      }, 400);
    }
    if (member.invitation_expires_at && new Date(member.invitation_expires_at) < /* @__PURE__ */ new Date()) {
      return c.json({
        success: false,
        error: "Esta invitaci\xF3n ha expirado"
      }, 400);
    }
    if (member.invitation_status === "revoked") {
      return c.json({
        success: false,
        error: "Esta invitaci\xF3n ha sido revocada"
      }, 400);
    }
    const owner = await c.env.DB.prepare("SELECT * FROM user_profiles WHERE id = ?").bind(member.owner_id).first();
    return c.json({
      success: true,
      email: member.email,
      role: member.role,
    ...
    console.error("Error validando invitaci\xF3n:", error);
    return c.json({
      success: false,
      error: error.message || "Error al validar invitaci\xF3n"
    }, 500);
  }
});
app21.get("/test", async (c) => {
  return c.json({
    success: true,
    message: "El endpoint de team-invitations est\xE1 funcionando",
    timestamp: (/* @__PURE__ */ new Date()).toISOString()
  });
});
app21.get("/me", async (c) => {
  const clerkUserId = c.get("clerkUserId");
  console.log("\u{1F50D} [GET /api/team-invitations/me] Llamado con clerkUserId:", clerkUserId);
  try {
    console.log("\u{1F50D} [GET /api/team-invitations/me] Buscando en DB con clerk_user_id:", clerkUserId);
    const member = await c.env.DB.prepare(`
        ...
        WHERE clerk_user_id = ?
          ...
          AND status = 'active'
      `).bind(clerkUserId).first();
    console.log("\u{1F4E6} [GET /api/team-invitations/me] Resultado de DB:", {
      found: !!member,
      email: member?.email,
      role: member?.role,
    ...
    if (!member) {
      console.log("\u274C [GET /api/team-invitations/me] No se encontr\xF3 team member activo");
      return c.json({
        success: false,
  ...
  try {
    ...
    const existing = await c.env.DB.prepare(`
      ...
      WHERE insight_type = ?
        AND title = ?
        AND status = 'active'
    `).bind(body.insight_type, body.title).first();
    if (existing) {
      await c.env.DB.prepare(`
...
var analytics_default = app22;

// src/routes/user-stores.ts
init_checked_fetch();
init_modules_watch_stub();
var app23 = new Hono2();
app23.get("/", async (c) => {
  ...
  try {
    ...
    const teamMemberships = await c.env.DB.prepare(`
        SELECT
          tm.owner_id,
          tm.role,
          tm.permissions,
...
app24.route("/api/webhooks", webhooks_default);
app24.post("/api/wompi/webhook", wompi_default);
app24.post("/api/subscriptions/webhook", subscriptions_default);
app24.route("/api/storefront", storefront_default);
app24.route("/stats", stats_default);
app24.post("/api/team-invitations/accept", async (c) => {
  try {
    const authHeader = c.req.header("Authorization");
    if (!authHeader || !authHeader.startsWith("Bearer ")) {
      return c.json({
    ...
    const token = authHeader.substring(7);
    const { decodeClerkToken: decodeClerkToken2 } = await Promise.resolve().then(() => (init_auth_helpers(), auth_helpers_exports));
    let clerkUserId;
    try {
      const payload = await decodeClerkToken2(token);
    ...
    if (!invitationToken || !userId) {
      return c.json({
        success: false,
        error: "Token de invitaci\xF3n requerido"
      }, 400);
    }
    const member = await c.env.DB.prepare("SELECT * FROM team_members WHERE invitation_token = ?").bind(invitationToken).first();
    if (!member) {
      return c.json({
        success: false,
        error: "Invitaci\xF3n no encontrada"
    ...
    await c.env.DB.prepare(`
        ...
        SET clerk_user_id = ?,
            ...
            updated_at = ?
        WHERE id = ?
      `).bind(userId, now, now, member.id).run();
    const updatedMember = await c.env.DB.prepare("SELECT * FROM team_members WHERE id = ?").bind(member.id).first();
    return c.json({
      success: true,
      teamMember: updatedMember,
      message: "Invitaci\xF3n aceptada exitosamente"
    });
  } catch (error) {
    console.error("Error aceptando invitaci\xF3n:", error);
    return c.json({
      success: false,
      error: error.message || "Error al aceptar invitaci\xF3n"
    }, 500);
  }
});
...
app24.post("/api/email/daily-reports", emailCronHandler);
app24.post("/api/email/subscription-reminders", emailCronHandler);
app24.post("/api/email/stock-alerts", emailCronHandler);
app24.post("/api/email/abandoned-carts", emailCronHandler);
app24.get("/api/team-invitations/validate", async (c) => {
  const token = c.req.query("token");
  if (!token) {
    return c.json({
      error: "Token requerido"
...
app24.use("/api/*", authMiddleware);
app24.route("/api/wompi", wompi_default);
app24.route("/api/subscriptions", subscriptions_default);
app24.route("/api/products", products_default);
app24.route("/api/customers", customers_default);
app24.route("/api/sales", sales_default);
app24.route("/api/categories", categories_default);
app24.route("/api/suppliers", suppliers_default);
app24.route("/api/purchase-orders", purchase_orders_default);
app24.route("/api/debtors", debtors_default);
app24.route("/api/user-profiles", user_profiles_default);
app24.route("/api/credit-payments", credit_payments_default);
app24.route("/api/offers", offers_default);
app24.route("/api/payment-transactions", payment_transactions_default);
app24.route("/api/shipping-zones", shipping_zones_default);
app24.route("/api/admin", admin_stats_default);
app24.route("/api/email", email_default);
app24.route("/loyalty-settings", loyalty_settings_default);
app24.route("/api/team-invitations", team_invitations_default);
app24.route("/api/team-members", team_invitations_default);
app24.route("/api/analytics", analytics_default);
app24.route("/api/user-stores", user_stores_default);
app24.notFound((c) => {
  return c.json({
    success: false,
    error: "Not Found",
...
async function scheduled(event, env, ctx) {
  ...
  const makeRequest = /* @__PURE__ */ __name(async (path) => {
    try {
      ...
      const response = await app24.fetch(request, env, ctx);
